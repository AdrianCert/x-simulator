...
class TestCallAndRetInstructions(unittest.TestCase):
    ...
    def test_call_execute(self):
        ...
        self.context.memory.write.assert_called_with(98, 0, 2)  # Verificăm dacă adresa de retur a fost scrisă în memorie
        self.assertEqual(self.context.registers["PC"], 200)  # Verificăm dacă PC-ul a fost setat la adresa de apel

    def test_ret_execute(self):
        self.context.memory.read.return_value = 300  # Adresa de retur citită din memorie

        Ret.execute(self.context)

        self.assertEqual(self.context.registers["PC"], 300)  # Verificăm dacă PC-ul a fost setat la adresa de retur
        self.assertEqual(self.context.registers["SP"], 102)  # Verificăm dacă SP-ul a fost actualizat corect
