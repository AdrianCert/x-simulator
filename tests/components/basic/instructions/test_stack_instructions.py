...
from src.xsim.components.basic.instructions.stack import Pop, Push
...
class TestPushAndPopInstructions(unittest.TestCase):
    ...
    def test_push_execute(self):
        ...
        self.context.memory.write.assert_called_with(
            98, 42, 2
        )  # Verificăm dacă valoarea a fost scrisă corect pe stivă
    ...
    def test_pop_execute(self):
        ...
        self.assertEqual(
            self.context.registers["eax"], 42
        )  # Verificăm dacă valoarea a fost încărcată corect în registrul "eax"
        self.assertEqual(
            self.context.registers["SP"], 100
        )  # Verificăm dacă SP-ul a fost actualizat corect
